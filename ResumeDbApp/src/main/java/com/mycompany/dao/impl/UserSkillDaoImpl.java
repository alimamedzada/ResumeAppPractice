...
public class UserSkillDaoImpl extends AbstractDAO implements UserSkillDaoInter {
	...
	private UserSkill getUserSkill(ResultSet rs) throws Exception {
		...
		int power = rs.getInt("power");
                ...
                return new UserSkill(userId,power);
//		return UserSkill.builder().id(userSkillId).user(User.builder().id(userId).build())
//				.skill(Skill.builder().id(skillId).name(skillName).build()).power(power).build();
	}
