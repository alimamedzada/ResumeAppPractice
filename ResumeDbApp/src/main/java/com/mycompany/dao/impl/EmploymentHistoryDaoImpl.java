...
public class EmploymentHistoryDaoImpl extends AbstractDAO implements EmploymentHistoryDaoInter {
    ...
    private EmploymentHistory getEmploymentHistory(ResultSet rs) throws Exception {
        ...
        return new EmploymentHistory(null, header, beginDate, jobDescription);
//        return EmploymentHistory.builder().id(null).beginDate(beginDate).endDate(endDate)
//                .jobDescription(jobDescription).user(User.builder().id(userId).build()).build();
    }
    ...
    public boolean updateEmploymentHistory(EmploymentHistory history) {
        try (Connection c = connect()) {
            ...
            stmt.setDate(2, (java.sql.Date) history.getBeginDate());
            stmt.setDate(3, (java.sql.Date) history.getEndDate());
